...
test("trust confirmation restores focus to its invoker", async ({ baseURL, browser }) => {
  ...
  try {
    ...
    const saved = ownerPage.waitForResponse(
      (r) => r.request().method() === "PUT" && r.url().includes("/api/notes/") && r.ok()
    );
    await ownerPage
      .getByRole("dialog", { name: "New note" })
      .getByRole("button", { name: "Create" })
...
test("viewer, offline, and recoverable error states remain accessible", async ({
  baseURL,
  browser
}) => {
  test.slow();
...
test("rotation progress, repair, and revoked states remain accessible", async ({
  baseURL,
  browser
}) => {
  const contexts: BrowserContext[] = [];
  ...
  try {
    ...
    const response = ownerPage.waitForResponse(
      (candidate) =>
        ...
        candidate.url().includes("/key-rotation")
    );
    ownerPage.once("dialog", (dialog) => {
      void dialog.accept();
    });
    await ownerPage
      .locator(".membership-list li", { hasText: viewer.username })
    ...
    await expect(viewerPage.locator(".collaboration-status")).toContainText(
