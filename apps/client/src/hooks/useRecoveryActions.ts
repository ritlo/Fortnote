...
export function useRecoveryActions(
  ...
  return useMemo(() => {
    const run = async (
      operation: (current: RecoverableSectionDraft) => Promise<void>
    ) => {
      if (!draft) return;
    ...
    const exportText = async (current: RecoverableSectionDraft) =>
      withDatabase(async (database) =>
        encryptedDraftJson(current, await database.listOutbox(current.userId))
      );
      ...
      cleanup: () =>
        run(async (current) => {
          await withDatabase(async (database) =>
            database.evictSectionCache(current.userId, 0)
          );
          retry();
        }),
      copy: () =>
...
async function withDatabase<T>(
  operation: (database: Awaited<ReturnType<typeof openFortnoteIndexedDb>>) => Promise<T>
): Promise<T> {
...
function encryptedDraftJson(
  draft: RecoverableSectionDraft,
  records: EncryptedOutboxRecord[]
): string {
